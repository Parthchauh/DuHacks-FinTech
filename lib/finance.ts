...
export function calculateRiskScore(assets: Asset[]): number {
    // Simple heuristic: Equity is riskier than bonds/cash
    // Normalize to 0-10 scale
    let weightedRisk = 0;
    assets.forEach(asset => {
        if (["VTI", "VXUS", "BTC"].includes(asset.ticker)) weightedRisk += (asset.balance * 0.8);
        if (["BND"].includes(asset.ticker)) weightedRisk += (asset.balance * 0.3);
        if (["USD"].includes(asset.ticker)) weightedRisk += (asset.balance * 0.0);
    });
